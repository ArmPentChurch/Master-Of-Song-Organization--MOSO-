...
from SPOT import DATABASE_FILEPATH, OUTPUT_FOLDER
...
class ModernSongManager:
    ...
    def open_db(self):
        with open(DATABASE_FILEPATH, 'r', encoding='utf-8') as db:
            self.allSongs:dict = load(db)
    ...
    def add_song(self,event=None):
        ...
        if song_num == "":
        ...
        elif not self.allSongs.get(song_num, False):
            messagebox.showerror("Error", "Could not find song, please input a valid song number")
        else:
            #TODO: Fix it!
            ...
            dupSong = SD.songChecker(songNum=song_num,book="")
